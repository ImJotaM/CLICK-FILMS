...
const p = document.getElementById('sem_negrito');
...
function openMenu() {
    if (MenuState == false) {
        ...
        RemoveToAvaliacaoAndP();
        RemoveToMenu();
    }
    MenuState = !MenuState;
}

function AddToAvaliacaoAndP(){
    avaliacao.classList.add('op_avaliacao');
    p.style.width = '50%';
}

function RemoveToAvaliacaoAndP(){
    avaliacao.classList.remove('op_avaliacao');
    p.style.width = '60%';
}
